...
mem_init(void)
{
	...
	/*
	 * envs是进程的概念，逻辑上，envs与进程一一对应。struct Env内含
	 * env_tf中断帧（寄存器、错误码以及中断编号），env_link（用于分配空闲进程
	 * 的单链表指针）、env_id（进程id，除了0~NENV的编号外，还加上了一些信息用于标示年代）
	 * env_parent_id（父进程的id）、env_type（进程标识，用于区分用户进程、系统进程）、
	 * env_status（进程状态：未分配、销毁中、可运行、运行中、不可运行）、env_runs（进程
	 * 被调度的次数）、env_pgdir（进程页目录）。
	 */
	...
	// or page_insert
	/*
	 * 初始化页配置器page_alloc的数据结构，执行完page_init函数后
	 * 申请内存都使用page_alloc，不能再使用boot_alloc。
	 */
	page_init();
	 ...
	 * 这里只映射了BSP的内核栈，其它AP内核栈的映射在pmap.c/mem_init_mp中完成
	 */
...
page_init(void)
{
	...
	// 占用第一页
	pages[0].pp_ref = 1;
	// 从第二页开始，整个base memory都是空闲的
	// 除了MPENTRY_PADDR所在的页面
	physaddr_t spec = ROUNDDOWN(MPENTRY_PADDR, PGSIZE);
	size_t i;
	...
	// 内核占用的部分
	// size_t kern_used = ((unsigned int)boot_alloc(0)-KERNBASE)/PGSIZE;
	size_t kern_used = PGNUM(PADDR(boot_alloc(0)));
	for (; i < kern_used; ++i) {
		pages[i].pp_ref = 1;
	}
	for (; i < npages; ++i) {
...
{
	// Fill this function in
	// 1. kvaddr->index of pgdir
	size_t pdIndex = PDX(va);
	pde_t* ppde = pgdir+pdIndex;
	// 2. 检查pde是否存在
	if (!(*ppde & PTE_P)) {
	...
	// 3. kvaddr->index of ptable
	size_t ptIndex = PTX(va);
	/*
	 * 这里很好地解释了将内核部分[KERNBASE, 0xfffffff)映射到[0, 0xffffffff-KERNBASE)
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	if (ppte == NULL)	return -E_NO_MEM;
	// 2. 检查pte是否已经被映射过了
	if ((*ppte & PTE_P)) {
		// 检查映射的物理页与pp所指向的是否为同一个
		if (page2pa(pp) == PTE_ADDR(*ppte)) {
			pp->pp_ref--;
		} else {
...
page_remove(pde_t *pgdir, void *va)
{
	...
	// 1. va->pte
	pte_t* ppte = pgdir_walk(pgdir, va, false);
	if (ppte == NULL || !(*ppte & PTE_P)) return;
	// 2. pte->pa->pp
	physaddr_t pa = PTE_ADDR(*ppte);
	struct PageInfo* pp = pa2page(pa);
	page_decref(pp);
...
{
	// LAB 3: Your code here.
	void* eva = ROUNDUP((void*) va + len, PGSIZE);
	void* temp_va = ROUNDDOWN((void*) va, PGSIZE);
	for (; temp_va < eva; temp_va += PGSIZE) {
		if ((uintptr_t)temp_va >= ULIM) { // 如果访问的地址超过ULIN
			user_mem_check_addr = temp_va < va ? (uintptr_t) va : (uintptr_t) temp_va;
			return -E_FAULT;
		}
		pte_t* ppte = pgdir_walk(env->env_pgdir, temp_va, 0);
		if ( (ppte==NULL) ||  ((*ppte)&(perm | PTE_P)) != (perm | PTE_P)) { // 如果访问的地址不存在或者用户没有访问权限
			user_mem_check_addr = temp_va < va ? (uintptr_t) va : (uintptr_t) temp_va;
			return -E_FAULT;
		}
	}
